...
db.version(1).stores({
    ...
    // NEW TABLE: Stores a single entry for the doctor's details
    doctor: 'id' // Use a fixed 'id' like 1 to ensure only one record
});
...
// NEW GLOBAL VARIABLE: Stores the currently loaded doctor's details
let doctorDetails = {
    docName: "Dr. SIVASANKAR MBBS",
    docSubtitle: "Physician, General Medicine, Reg. No: XXXXX",
    docContact: "123 Main Street, City, State | Phone: (123) 456-7890 | Email: example@clinic.com",
};
...
async function loadDoctorDetails() {
    try {
        const details = await db.doctor.get(1); // Try to get the fixed record with id=1
        if (details) {
            doctorDetails = details;
            // Prefill the modal inputs when the modal is opened
            document.getElementById('docName').value = details.docName || '';
            document.getElementById('docSubtitle').value = details.docSubtitle || '';
            document.getElementById('docContact').value = details.docContact || '';
        } else {
            // If no record exists, use the default and add the default details to the inputs
            document.getElementById('docName').value = doctorDetails.docName;
            document.getElementById('docSubtitle').value = doctorDetails.docSubtitle;
            document.getElementById('docContact').value = doctorDetails.docContact;
        }
    } catch (error) {
...
window.saveDoctorDetails = async function() {
    const newDetails = {
        id: 1, // Fixed ID for single record
        docName: document.getElementById('docName').value.trim(),
        docSubtitle: document.getElementById('docSubtitle').value.trim(),
        docContact: document.getElementById('docContact').value.trim()
    };

    if (!newDetails.docName) {
        return alert("Doctor's Full Name & Title is required.");
    }

    try {
        // Use PUT to insert (if id=1 doesn't exist) or update (if id=1 exists)
        await db.doctor.put(newDetails);
        doctorDetails = newDetails; // Update the global variable
        
        // Use Bootstrap's way to close the modal after a successful save
        const modal = bootstrap.Modal.getInstance(document.getElementById('doctorModal'));
        modal.hide();
        
        showMsg('Doctor details saved successfully!');
    } catch (error) {
        console.error("Failed to save doctor details:", error);
        showMsg('Failed to save doctor details.', true);
    }
}
...
function updateSaveButton(isUpdate) {
    // ... (rest of updateSaveButton is the same)
...
function updateNewRecordButton(isVisible) {
    // ... (rest of updateNewRecordButton is the same)
...
function setupDosageToggle(row) {
    // ... (rest of setupDosageToggle is the same)
...
function addMedicineRow(p = {}) {
    // ... (rest of addMedicineRow is the same)
...
window.removeMedicineRow = function(buttonElement) {
    // ... (rest of removeMedicineRow is the same)
...
window.addEventListener('load', () => {
    ...
    loadDoctorDetails(); // NEW: Load doctor details on startup
    ...
    // NEW: Load details into modal just before it's shown
    const doctorModalEl = document.getElementById('doctorModal');
    doctorModalEl.addEventListener('show.bs.modal', loadDoctorDetails);
});

// --- Data Loading and Filtering (MOBILE-FOCUSED) ---
async function loadAllVisitsInternal() {
    // ... (rest of loadAllVisitsInternal is the same)
    try {
...
async function loadVisitsByMobile() {
    // ... (rest of loadVisitsByMobile is the same)
...
function updateVisitDropdown(list) {
    // ... (rest of updateVisitDropdown is the same)
...
window.applyDateFilter = function() {
    // ... (rest of applyDateFilter is the same)
...
window.clearFilters = function() {
    // ... (rest of clearFilters is the same)
...
async function loadVisitDetails(id) {
    // ... (rest of loadVisitDetails is the same)
    try {
    ...
    try {
        if (currentVisitId) {
            ...
            if (existing) {
                ...
                const addedId = await db.visits.add(patient);
                // After adding a new record, we need to know its ID to potentially load it, but for simplicity, we just reload data later.
...
function getPatientDataFromForm() {
    // ... (rest of getPatientDataFromForm is the same)
    const name = document.getElementById('name').value.trim();
    const mobile = document.getElementById('mobile').value.trim();
...
// --- .DOC GENERATION FUNCTIONS (MODIFIED TO USE doctorDetails) ---
function generateDocContent(data) {
    // USE THE GLOBAL doctorDetails VARIABLE
    const docName = doctorDetails.docName;
    const docSubtitle = doctorDetails.docSubtitle;
    const docContact = doctorDetails.docContact;
    ...
    const style = `
        ...
        .header p { margin: 3px 0; font-size: 14px; color: #666; white-space: pre-wrap; } /* Allow wrapping and preserved line breaks */
...
function downloadFile(content, filename) {
    // ... (rest of downloadFile is the same)
...
function downloadRecordAsDoc(patientData) {
    // ... (rest of downloadRecordAsDoc is the same)
...
window.downloadCurrentRecord = function() {
    // ... (rest of downloadCurrentRecord is the same)
...
window.printCurrentRecord = function() {
    // ... (rest of printCurrentRecord is the same)
    const patientData = getPatientDataFromForm();
...
window.backupData = async function() {
    // ... (rest of backupData is the same)
    try {
...
window.restoreData = function(event) {
    // ... (rest of restoreData is the same)
